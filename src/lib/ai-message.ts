...
[마무리 — 포함된 링크 안내]
아래 사용자 메시지의 "포함할 링크" 목록에 실제로 있는 항목만, 그 각각을 자연스러운 문장으로
소개하며 마무리 문단에 자연스럽게 포함시켜(URL은 주어진 그대로 정확히 인용 — 축약·변형·재구성
하지 마). 링크 종류별 소개 톤 참고(그대로 베끼지 말고 자연스럽게 변형해도 됨, 그 종류가 목록에
있을 때만 참고할 것):
- 프로그램티칭: "자세한 내용은 아래에서 확인해보세요"
- 이벤트: "이벤트 소식은 아래 링크에서 확인해보세요"
- 검사결과: "검사 결과는 아래에서 확인 가능합니다"
- 추천링크: "링크는 아래에서 확인해보세요"
링크가 여러 개면 각각을 자연스럽게 순서대로 소개해(하나로 뭉뚱그리지 말 것). 위 4가지는 어디까지나
표현 톤을 보여주는 참고 예시일 뿐, "포함할 링크" 목록에 없는 종류는 절대 언급하지 마(예: 검사결과가
목록에 없는데 "검사 결과도 확인해보세요"처럼 존재하지 않는 링크를 지어내서 안내하면 안 됨). "포함할
링크"가 없거나 "없음"이면 이런 클로징을 절대 억지로 넣지 말고, 특정 URL이나 예약 안내 없이 그냥
메시지 목적에 맞게 자연스럽게 마무리해.
...
6. "포함할 링크" 목록에 있는 항목은 빠짐없이 전부 자연스럽게 소개했고 URL을 정확히 그대로
   인용했는가? 반대로 그 목록에 없는 링크 종류(검사결과/이벤트/추천링크/프로그램티칭 등)를
   언급하지는 않았는가? 목록이 "없음"이라면 예약 안내나 특정 URL 언급을 억지로 넣지 않았는가?
...
[3-1. 분량 제한 — 카카오톡 대화창 "더보기" 방지 (반드시 지킬 것)]
patientMessage는 카카오톡 대화창에서 "더보기"를 누르지 않고 한 번에 다 보여야 해. 공백
포함 전체 분량을 400자 이내로 작성해(추천링크/마감설문 링크처럼 URL을 그대로 포함해야 하는
경우 그 URL 문자열의 길이도 400자 안에 함께 포함해서 세). 400자는 엄격한 상한선이야 —
넘기지 마. 분량을 줄일 때는 [2. 설문 데이터 활용 원칙]의 핵심 소재(설문 기반 공감 포인트,
복용법, 링크처럼 실제 정보)는 유지하고, 그 대신 수식어·중복 설명·부연 문장부터 먼저
쳐내(예: "정말", "너무", "꼭", 같은 말을 다르게 반복하는 문장, 이미 앞 문장에서 전달한
내용을 다시 풀어쓰는 문장).
...
6. [3-1. 분량 제한]대로 patientMessage 전체 글자수(공백 포함, URL 포함)를 실제로 세어봤을 때
   400자 이내인가? 넘었다면 핵심 정보(설문 공감 포인트, 복용법, 링크)는 남기고 수식어·중복
   설명부터 줄여서 400자 이내로 다시 써.
위 6가지 중 하나라도 걸리면 반드시 고친 뒤 최종본만 출력해.`;

const TRIAL_MESSAGE_TYPE_PROMPT: Record<"TRIAL_WELCOME" | "TRIAL_DAY2", string> = {
  TRIAL_WELCOME:
...
// 마감톡 프롬프트(task.md Phase 2-2)만 처방별 마감설문 링크(/refer/exit/[prescriptionId])가
// 필요해 함수로 분리한다 — 기존 forms.gle 고정 링크(TRIAL_DEADLINE_SURVEY_LINK) 대체.
function buildTrialDeadlinePrompt(exitSurveyUrl: string): string {
  return (
    ...
    `진행된다는 설명 5) 마감설문 링크 제공(반드시 이 링크를 그대로 포함: ${exitSurveyUrl}) ` +
    ...
    `[3-1. 분량 제한] 400자 계산에는 위 마감설문 링크(${exitSurveyUrl}, ${exitSurveyUrl.length}자)도 ` +
    "포함해서 세 — 링크 앞뒤 문장을 그만큼 더 짧게 써야 전체가 400자 안에 들어간다."
  );
}
...
export async function generateTrialMessageDraft(
  ...
  // TRIAL_DEADLINE 전용(task.md Phase 2-2) — 처방별 마감설문 링크. 호출측(program-events/
  // generate/route.ts)이 항상 채워서 전달한다.
  ...
  const instruction =
    taskType === "TRIAL_DEADLINE" ? buildTrialDeadlinePrompt(exitSurveyUrl ?? "") : TRIAL_MESSAGE_TYPE_PROMPT[taskType];
