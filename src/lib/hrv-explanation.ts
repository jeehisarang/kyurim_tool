...
// 되는" 느낌이라는 피드백으로 AI 개인화 버전(커밋 48dbc6e)으로 롤백, 단 근거설명 문장은
// 덜어냄). 카테고리 하나당 완전히 독립된 AI 호출로 만든다 — 프롬프트 자체에 다른 카테고리
// 정보를 아예 안 주므로 "다른 카테고리 언급 없이"가 프롬프트 지시가 아니라 구조적으로
// 보장된다(섞일 재료 자체가 없음, 이 구조는 고정사전 라운드에서도 유지됐던 부분이라 롤백
// 대상이 아니다). treatmentPrinciple이 null인 카테고리는 호출 자체를 만들지 않는다(창작
...
[절대 금지]
- "~때문에", "~이유로", "~증상에 맞춰", "~해서"처럼 근거나 이유를 설명하는 연결어를 쓰지
  말 것 — 이 카드는 방향을 담백하게 알려주는 카드이지, 이유를 설명하는 카드가 아닙니다.
...
본문만 출력하세요(제목/마커/따옴표 없이 1문장 내외로 바로 시작).`;
