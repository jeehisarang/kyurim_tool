...
import { TREATMENT_KEYWORD_HANJA, extractTreatmentKeywords, extractPrescriptionNames } from "@/lib/hrv-health-report";
...
// "공통 생활관리" 문단만 담당 — 카테고리별 치료 방향은 이 파일 아래쪽 별도 AI 호출
// generateCategoryTreatmentCards가 카드마다 독립적으로 만든다), tcmInterpretation은
...
추가로 카드4(한의건강해석)와 카드5(경향지속시)는 문장만 쭉 나열되면 여러 카테고리가 겹칠 때
핵심이 눈에 안 들어온다는 피드백이 있었습니다 — 위 핵심 문장 강조와는 별개로, 카테고리명·
핵심 패턴명(예: 과로형만성스트레스)·결정적 증상 조합처럼 눈에 띄어야 할 단어/짧은 구절에
한해 별표로 감싸는 단어 단위 강조를 추가로 쓰세요(예: "**과로형만성스트레스** 경향이
지속되면"). 단, 남용하지 마세요 — 카드(문단) 하나당 이런 단어 단위 강조는 1~2곳까지만
입니다. 이미 핵심 문장 강조 안에 포함된 단어는 이중으로 감싸지 마세요.
...
(c) 미병(未病) 언급: (a)나 (b) 문장 어딘가에 "미병(未病)" 개념을 자연스럽게 1회 녹여
    쓰세요 — 강제로 별도 문장을 끼워넣지 말고 이미 쓰는 문장의 흐름 안에 자연스럽게
    포함시킬 것. 예: "몸이 아직 병은 아니지만 보내는 신호 — 미병(未病) — 인 [패턴/카테고리
    명] 경향을 살펴볼 필요가 있습니다." 문장이 부자연스러워지면(어색한 나열, 억지 삽입)
    안 됩니다 — 자연스러움이 우선입니다.
...
- 값이 정상적으로 주어졌다면 이 카드에 반드시 포함하세요. 자율신경균형도 5유형/맥박다양성은
  ...
  가까운 패턴입니다" 형태로(이 표현 자체 필수 포함) 서술하세요.
  🚫 절대 금지(중복 문장 버그): 구역 이야기는 이 문장 하나로 딱 한 번만 하세요. "이번 검사
  구역이 OO에 가까운 패턴으로" 같은 문장을 먼저 쓰고, 뒤이어 "기기 분석 기준으로는 OO 구역에
  가까운 패턴입니다"를 또 쓰는 식으로 같은 결론을 다른 표현으로 두 번 말하는 것은 명백한
  오류입니다(예: "이번 검사 구역이 과로형만성스트레스에 가까운 패턴으로, 기기 분석 기준으로는
  과로형만성스트레스 구역에 가까운 패턴입니다" — 이렇게 쓰지 말 것). 구역명은 판독 결과에 주어진
...
- 카드1에 미병(未病) 언급이 자연스럽게 녹아 있는가? 문장 흐름을 어색하게 만들지 않는가
  (억지로 끼워넣은 느낌이 나면 다시 쓸 것)?
...
- 카드4/카드5의 단어 단위 강조가 카드당 1~2곳을 넘지 않는가(남용하지 않았는가)?
- 구역명을 언급했다면 "기기 분석 기준으로는 ~에 가까운 패턴입니다" 표현을 포함했는가?
  맥박다양성은 "본인의 이전 추세와 비교해 참고"하는 취지로만 썼는가?
- 구역 관련 결론("~구역에 가까운 패턴")을 다른 표현으로 앞서 한 번 더 말하지 않았는가
  (같은 문장이 표현만 바뀌어 두 번 나오는 중복 문장 버그가 없는지 다시 확인)?
...
// 중복 문장 버그(task.md 실사용 피드백) — "이번 검사 구역이 OO에 가까운 패턴으로, 기기 분석
// 기준으로는 OO 구역에 가까운 패턴입니다"처럼 같은 결론을 다른 표현으로 두 번 말하는 경우가
// 실측 확인됐다. hasDuplicatedEmphasisSentence(강조 문장 중복)와는 다른 패턴이라(여긴 강조
// 마커 없이 표현만 다르게 두 번 등장) 별도 검사가 필요하다 — "구역에 가까운 패턴"이라는
// 고정 구절이 2회 이상 등장하면 위반으로 본다(정상적으로는 필수 문구 1회만 등장해야 함).
function violatesZoneRestatementRule(tcmInterpretation: string, hasImage: boolean): boolean {
  if (!hasImage) return false;
  const matches = tcmInterpretation.match(/구역에\s*가까운\s*패턴/g);
...
export async function generateHrvExplanation(input: HrvExplanationInput): Promise<HrvExplanationSections> {
  ...
  const evaluate = (s: HrvExplanationSections) => {
    return {
      ...
      zoneRestatementViolation: violatesZoneRestatementRule(s.tcmInterpretation, hasImage),
  ...
  if (firstViolations.zoneRestatementViolation) {
    correctionInstructions.push(
      "직전 응답의 카드4에서 자율신경균형도 구역 결론을 다른 표현으로 두 번 말했습니다(예: " +
        "'이번 검사 구역이 OO에 가까운 패턴으로'라고 먼저 쓴 뒤 '기기 분석 기준으로는 OO 구역에 " +
        "가까운 패턴입니다'를 또 씀). 이번에는 구역 이야기를 '기기 분석 기준으로는 ~구역에 " +
        "가까운 패턴입니다' 문장 하나로만 쓰고, 같은 내용을 다른 문장으로 앞서 반복하지 마세요.",
    );
  }
  ...
  if (retriedViolations.zoneRestatementViolation) {
    throw new Error("자율신경균형도 구역 결론이 다른 표현으로 중복 서술되는 버그가 재시도 후에도 남아있습니다.");
  }
...
// 카드7 카테고리별 치료방향 카드(task.md "환자용/원장용 분리") — 카테고리 하나당 완전히
// 독립된 AI 호출로 만든다(다른 카테고리 정보를 아예 안 줌, 섞일 재료 자체가 없음). 이번
// 라운드부터 한 호출에서 doctorText(기존 그대로, 변증명+한자+방제 계열 — applyDeterministicBold
// 대상 아님, 한자 병기는 hrv.ts에서 별도로 annotateTreatmentKeywords가 처리)와 patientText
// (신규, 전문용어 전면 금지)를 동시에 출력한다 — 토큰/비용 절약을 위해 별도 호출 2번이 아닌
// 마커 2개로 한 응답에서 분리한다. treatmentPrinciple이 null인 카테고리는 호출 자체를
// 만들지 않는다(창작 방지). categoryCode는 카드4/카드7 공통 고정 색상/아이콘 매핑
// (tcm-category-visuals.ts) 조회 키.
export type CategoryTreatmentCard = { categoryCode: string; categoryLabel: string; doctorText: string; patientText: string };

const CARD_TREATMENT_MARKERS = { doctorText: "[의사용]", patientText: "[환자용]" } as const;
...
이 카드는 원장이 상담 중 참고하는 "의사용"과 환자 화면에 그대로 노출되는 "환자용" 두 버전이
필요합니다. 아래 마커를 정확히 그대로 각 줄 맨 앞에 쓰고, 그 다음 줄부터 본문을 이어 쓰세요.
마커 2개와 그 본문 외 다른 텍스트는 절대 출력하지 마세요.

[출력 형식]
${CARD_TREATMENT_MARKERS.doctorText}
(의사용 본문)
${CARD_TREATMENT_MARKERS.patientText}
(환자용 본문)

[의사용 작성 지침]
[치료원칙]에는 이 카테고리의 한의학적 치료 원칙 키워드와 참고할 수 있는 대표처방이 함께
적혀 있습니다. 이 내용을 담백하게 1문장 내외로 정리하세요 — 왜 이 치료 방향이 이 카테고리에
맞는지 설명하거나 근거를 덧붙이지 마세요. 키워드와 대표처방을 자연스럽게 나열하는 수준이면
충분합니다.
예시(형식만 참고, 내용은 실제 [치료원칙]을 따를 것): "소간해울·이기해울·청간사화·양심안신의
치료 방향을 고려할 수 있으며, 시호소간산·가미소요산 계열이 참고될 수 있습니다."
카테고리 제목은 화면에 이미 별도로 표시되므로, 본문에 카테고리명을 마크다운 강조(*나 **)로
감싸 반복할 필요 없습니다.

[환자용 작성 지침]
환자가 직접 읽는 화면입니다 — [치료원칙]에 적힌 변증명(예: 화담이수, 소간해울)이나 대표처방명
(예: 시호소간산), 한자를 단 하나도 그대로 옮기지 마세요. 전문용어는 전부 쉬운 우리말로 풀어
설명해야 합니다(예: "화담이수" → "몸에 쌓인 습한 기운을 배출하는").
1. 문장 구조: "[증상 상태 설명]. [치료 방향]을 고려할 수 있습니다." 형태로 1~2문장만 쓰세요.
2. [체크된 증상]이 "없음"이 아니면, 그중 이 카테고리와 자연스럽게 어울리는 표현을 문장 안에
   녹이고 그 부분을 **볼드**로 감싸세요(실제로 주어진 내용과 무관한 증상을 지어내지 말 것).
3. 치료 방향을 설명하는 핵심 어구("~한 치료", 예: "기운을 보충하는 치료", "습한 기운을
   배출하는 치료")도 **볼드**로 감싸세요. 카드 하나당 볼드는 이 두 곳(증상 표현 + 치료
   방향 어구)까지만 — 남용하지 마세요.
4. "구체적인 치료는 상담을 통해 결정됩니다" 같은 공통 안내 문구는 절대 쓰지 마세요(화면
   하단에 별도로 한 번만 고정 노출됩니다 — 여기서 또 쓰면 중복입니다).

[핵심 원칙 — 두 버전 공통]
- [치료원칙]에 없는 다른 치료법이나 효능·통계를 창작하지 말 것
- 질병명·확정 진단을 쓰지 말고 유보적 어조를 유지할 것
- 구체적인 처방/치료 기간을 확정하듯 쓰지 말 것(그건 원장 상담의 몫)
- "AI"라는 단어를 쓰지 말 것
- "~때문에", "~이유로", "~증상에 맞춰", "~해서"처럼 근거·이유를 설명하는 연결어를 쓰지
  말 것 — 방향을 담백하게 알려주는 카드이지, 이유를 설명하는 카드가 아닙니다.
- 다른 카테고리를 언급하거나 비교성 표현을 쓰지 말 것

[자체검토 — 출력 전 스스로 점검]
- [환자용]에 변증명·방제명·한자가 단 하나라도 남아있는가? 있으면 전부 쉬운 우리말로 바꿀 것
- [환자용]에 체크된 증상 표현과 치료 방향 핵심 어구가 각각 정확히 1곳씩 **볼드**로
  감싸져 있는가(카드당 2곳까지, 남용하지 않았는가)?
- [의사용]/[환자용] 둘 다 1~2문장 내외로 담백한가?`;

// AI 응답을 마커 기준으로 doctorText/patientText로 분리한다. 마커가 없거나 순서가 어긋나면
// throw해서 호출측이 실패로 처리하게 한다(generateHrvExplanation과 동일 원칙).
function parseCategoryTreatmentSections(text: string): { doctorText: string; patientText: string } {
  const order = [
    ["doctorText", CARD_TREATMENT_MARKERS.doctorText],
    ["patientText", CARD_TREATMENT_MARKERS.patientText],
  ] as const;
  const indices = order.map(([, marker]) => text.indexOf(marker));
  if (indices.some((idx) => idx === -1)) {
    throw new Error("치료방향 카드 응답에서 의사용/환자용 마커를 찾지 못했습니다.");
  }
  if (indices[1] <= indices[0]) {
    throw new Error("치료방향 카드 응답의 마커 순서가 올바르지 않습니다.");
  }
  return {
    doctorText: text.slice(indices[0] + order[0][1].length, indices[1]).trim(),
    patientText: text.slice(indices[1] + order[1][1].length).trim(),
  };
}

// 환자용 텍스트에 한의학 전문용어(변증명·방제명·한자)가 남아있는지 검증한다(task.md — 프롬프트
// 지시만으로는 신뢰 못 한다는 기존 원칙과 동일). CJK 한자 문자가 하나라도 있으면 즉시 위반—
// 변증명을 한글로 그대로 옮긴 경우는 이 카테고리의 키워드/대표처방 목록과 문자열 대조로 잡는다.
const CJK_REGEX = /[一-鿿]/;

function violatesPatientTextJargonRule(patientText: string, keywords: string[], prescriptionNames: string[]): boolean {
  if (CJK_REGEX.test(patientText)) return true;
  if (keywords.some((k) => patientText.includes(k))) return true;
  if (prescriptionNames.some((p) => patientText.includes(p))) return true;
  return false;
}

async function callCategoryTreatmentModel(
  ...
  const client = new OpenAI();
  const checkedSymptomsText = checkedSymptoms.length > 0 ? checkedSymptoms.map((s) => `- ${s}`).join("\n") : "없음";
  const userMessage =
  ...
  return parseCategoryTreatmentSections(text);
}

// 환자용 텍스트에 전문용어가 남아있으면 1회 재시도하고, 그래도 남아있으면 실패 처리한다
// (throw) — doctorText를 그대로 노출하는 폴백은 절대 쓰지 않는다(그 자체가 전문용어 원문이라
// task.md가 막으려는 문제를 그대로 재현하게 됨. 위반된 텍스트를 저장하는 것보다 이번 카드
// 생성 실패가 낫다는 기존 안전 원칙 그대로 적용).
async function generateCategoryTreatmentCard(
  categoryLabel: string,
  treatmentPrinciple: string,
  checkedSymptoms: string[],
): Promise<{ doctorText: string; patientText: string }> {
  const keywords = extractTreatmentKeywords(treatmentPrinciple);
  const prescriptionNames = extractPrescriptionNames(treatmentPrinciple);

  const first = await callCategoryTreatmentModel(categoryLabel, treatmentPrinciple, checkedSymptoms);
  if (!violatesPatientTextJargonRule(first.patientText, keywords, prescriptionNames)) return first;

  const retried = await callCategoryTreatmentModel(
    categoryLabel,
    treatmentPrinciple,
    checkedSymptoms,
    "직전 응답의 [환자용]에 한의학 전문용어(변증명·방제명·한자)가 남아있었습니다. 이번에는 " +
      "전문용어를 절대 그대로 쓰지 말고 전부 쉬운 우리말로 풀어서 설명하세요.",
  );
  if (violatesPatientTextJargonRule(retried.patientText, keywords, prescriptionNames)) {
    throw new Error(`카테고리별 치료방향 카드의 환자용 텍스트에 전문용어가 재시도 후에도 남아있습니다: ${categoryLabel}`);
  }
  return retried;
}
 ...
 * 것이 창작보다 안전하다. tcmCategoryProfile이 null(증상 패턴 프로필 없음)이면 빈 배열 —
 * 이 경우 카드7(공통 생활관리)만 노출된다. checkedSymptomsByCategoryCode는 환자용 텍스트가
 * 자연스럽게 녹여 쓸 카테고리별 체크 증상 문구(hrv.ts에서 조립).
 */
export async function generateCategoryTreatmentCards(
  ...
  checkedSymptomsByCategoryCode: Record<string, string[]>,
  ...
  return Promise.all(
    withPrinciple.map(async (c) => {
      const result = await generateCategoryTreatmentCard(
        c.patientLabel,
        c.treatmentPrinciple,
        checkedSymptomsByCategoryCode[c.categoryCode] ?? [],
      );
      return { categoryCode: c.categoryCode, categoryLabel: c.patientLabel, ...result };
    }),
  );
}

// 클로징 헤드라인(task.md "미병 프레임 복원") — 카드7 맨 아래, 그날 체크된 후보 카테고리
// 전체를 "따로가 아니라 함께 나타난다"는 흐름으로 엮고 미병(未病)/치미병(治未病) 정의로
// 마무리하는 신규 독립 AI 호출. 카테고리별 치료방향 카드와는 별개 — 카테고리 목록 전체를
// 한 번에 받아 리포트를 닫는 역할만 한다.
const CLOSING_HEADLINE_SYSTEM_PROMPT = `당신은 한의원 "건강 리포트"의 마지막 마무리 문구를 쓰는 카피라이터입니다.
이 문구는 카드7(치료 방향) 맨 아래, 리포트 전체를 닫는 역할을 합니다.

[체크된 카테고리 목록]에 나열된 카테고리들이 "따로가 아니라 함께" 나타나고 있다는 흐름으로
엮은 1~2문장을 쓰고, 이어서 미병(未病)·치미병(治未病) 개념을 짧게 정의하며 마무리하는 문장을
1개 추가하세요(총 2~3문장 내외).

[체크된 카테고리 목록]이 1개뿐이면 "여러 신호가 함께" 같은 복수 표현을 쓰지 말고, 그 카테고리
하나의 신호를 다루는 자연스러운 단수 표현으로 쓰세요.

예시 톤(카테고리 4개일 때, 내용은 실제 [체크된 카테고리 목록]을 따를 것): "긴장, 기력저하,
순환저하가 따로가 아니라 함께 나타나는 지금, 미병(未病)—아직 병은 아니지만 균형이 무너지기
시작한 상태의 신호입니다. 병이 되기 전에 미리 다스리는 치미병(治未病)이, 지금 필요한
치료입니다."

[절대 금지]
- 미병/치미병 정의를 왜곡하지 말 것 — "질병으로 확정되기 전, 몸의 불균형 신호를 미리
  포착해 예방적으로 관리한다"는 개념을 그대로 유지할 것
- 과도한 불안을 조성하는 표현("위험합니다", "큰 병이 될 수 있습니다", "심각한 질환" 등)을
  쓰지 말 것
- 질병명·확정 진단을 쓰지 말 것
- "AI"라는 단어를 쓰지 말 것
- [체크된 카테고리 목록]에 없는 카테고리를 지어내지 말 것

핵심 구절("미병(未病)", "치미병(治未病)", "따로가 아니라 함께" 등)에 한해 **볼드**로 감싸도
됩니다 — 문단 전체나 여러 문장을 감싸지 말 것.

본문만 출력하세요(제목/마커/따옴표 없이 바로 문장으로 시작, 2~3문장 내외).`;

async function callClosingHeadlineModel(candidateLabels: string[], extraInstruction?: string): Promise<string> {
  const client = new OpenAI();
  const userMessage =
  ...
  const response = await client.chat.completions.create({
    model: TEXT_MODEL,
    messages: [
      { role: "system", content: CLOSING_HEADLINE_SYSTEM_PROMPT },
      { role: "user", content: userMessage },
    ],
    max_tokens: 300,
  });
  const text = response.choices[0]?.message?.content?.trim();
  if (!text) throw new Error("클로징 헤드라인 생성 실패(빈 응답).");
  return text;
}

// 과도한 불안 조성 표현 검사 — 카드5/클로징 헤드라인 공통 원칙(코드로 결정적으로 검증
// 가능한 최소 안전장치만, 뉘앙스 판단까지는 못 잡지만 명백한 위반 문구는 걸러낸다).
const ANXIETY_WORDS = ["위험합니다", "큰 병", "심각한 질환", "즉시 치료하지 않으면"];

function violatesAnxietyRule(text: string): boolean {
  return ANXIETY_WORDS.some((w) => text.includes(w));
}

function violatesSingularPluralRule(text: string, candidateCount: number): boolean {
  if (candidateCount !== 1) return false;
  return text.includes("여러 신호가 함께") || text.includes("함께 나타나는");
}

/**
 * 클로징 헤드라인을 생성한다. candidateLabels가 빈 배열이면(증상 패턴 프로필 없음) null —
 * 이 경우 카드7 하단 클로징 카드 자체를 숨긴다(에러 아님, 조건부 렌더링).
 */
export async function generateClosingHeadline(candidateLabels: string[]): Promise<string | null> {
  if (candidateLabels.length === 0) return null;

  const first = await callClosingHeadlineModel(candidateLabels);
  const firstOk = !violatesAnxietyRule(first) && !violatesSingularPluralRule(first, candidateLabels.length);
  if (firstOk) return first;

  const retried = await callClosingHeadlineModel(
    candidateLabels,
    violatesSingularPluralRule(first, candidateLabels.length)
      ? "카테고리가 1개뿐인데 '여러 신호가 함께' 같은 복수 표현을 썼습니다. 자연스러운 단수 표현으로 바꾸세요."
      : "과도하게 불안을 조성하는 표현이 있었습니다. 더 담백하고 유보적인 어조로 다시 쓰세요.",
  );
  if (violatesAnxietyRule(retried) || violatesSingularPluralRule(retried, candidateLabels.length)) {
    throw new Error("클로징 헤드라인이 재시도 후에도 안전 기준을 위반합니다.");
  }
  return retried;
}
