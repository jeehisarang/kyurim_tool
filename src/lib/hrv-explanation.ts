...
// 별도 필드에 저장), lifestyleGuide→treatmentAndLifestyle(카드7), tcmInterpretation은
...
카드7(치료방향양생) — "치미병(治未病)을 위한 치료 방향과 생활관리"입니다. [증상 패턴
프로필]에 "치료원칙: X"가 주어진 카테고리가 있으면 그 치료원칙(예: 화담이수, 건비화습 등)을
자연스럽게 녹여 구체적 치료 방향으로 언급하세요. "(치료원칙 미입력)"인 카테고리는 치료
방향을 절대 창작하지 말고 생활관리만 서술하세요. [학술 근거]와 [일반 배경지식]에 있는
내용만 참고해서 생활관리 부분을 재구성하고, 그 안에 없는 새로운 의학적 효능/통계를 창작하지
말 것. "연구에 따르면"이 아니라 "임상적으로", "~하는 경우가 많습니다" 톤만 쓸 것. 복용 중인
약물을 줄이라는 뉘앙스는 절대 쓰지 말 것. [학술 근거]가 "없음"이면 구체적 방법을 창작하지
말고 아주 짧고 담백하게만 쓸 것.
...
- "(치료원칙 미입력)"인 카테고리에 카드7에서 구체적 치료법을 창작하지 않았는가?
...
// 증상 패턴 프로필의 후보 카테고리 전부가 치료원칙 미입력 상태인데도 카드4/카드7에서 구체적
...
export async function generateHrvExplanation(input: HrvExplanationInput): Promise<HrvExplanationSections> {
  ...
  const evaluate = (s: HrvExplanationSections) => {
    const combinedTreatmentText = `${s.tcmInterpretation} ${s.treatmentAndLifestyle}`;
    return {
      ...
      inventedTreatmentViolation: violatesInventedTreatmentRule(combinedTreatmentText, input.tcmCategoryProfile),
